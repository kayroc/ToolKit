...
public final class KeyBoardUtils {
    ...
    public static boolean isSoftInputVisible(
        ...
        return getContentViewInvisibleHeight(activity.getWindow()) >= minHeightOfSoftInput;
    }
    ...
    private static int getContentViewInvisibleHeight(final Window window) {
        try {
            ...
            return Math.abs(contentView.getRootView().getBottom() - rect.bottom);
        } catch (Exception e) {
            LogUtils.e(e.getMessage());
            return 0;
        }
    ...
    public static boolean registerSoftInputChangedListener(
        ...
        try {
            ...
            contentView.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
                if (listener != null) {
                    // 获取高度
                    int height = getContentViewInvisibleHeight(activity.getWindow());
                    // 判断是否相同
    ...
    public static void fixSoftInputLeaks(final Context context) {
        ...
        try {
            ...
            String[] leakViews = new String[]{
                "mCurRootView", "mServedView",
                "mNextServedView", "mLastSrvView"
            };
            for (String leakView : leakViews) {
                try {
                    Field leakViewField = imm.getClass().getDeclaredField(leakView);
                    if (!leakViewField.isAccessible()) {
                        leakViewField.setAccessible(true);
                    }
                    Object object = leakViewField.get(imm);
    ...
    public static boolean openKeyboardDelay(final long delayMillis) {
        sMainHandler.postDelayed(KeyBoardUtils::openKeyboard, delayMillis);
        return true;
    }
    ...
    public static boolean closeKeyboardDelay(final long delayMillis) {
        sMainHandler.postDelayed(KeyBoardUtils::closeKeyboard, delayMillis);
        return true;
    }
