package kayroc.android.utils.screenshot;
...
import kayroc.android.utils.ContentResolverUtils;
import kayroc.android.utils.HandlerUtils;
import kayroc.android.utils.LogUtils;
import kayroc.android.utils.ResourceUtils;
...
public final class ScreenshotUtils {
    ...
    public static void handleMediaContentChange(
        ...
        try {
            if (cursor == null) {
                LogUtils.d("搜索失败 uri:" + contentUri + ", projection: " + Arrays.toString(MEDIA_PROJECTIONS));
                return;
            }
            if (!cursor.moveToFirst()) {
                LogUtils.d("搜索成功 但无符合条件数据 uri:" + contentUri + ", projection: " + Arrays.toString(MEDIA_PROJECTIONS));
                return;
            }
            // 获取数据
    ...
    public static boolean handleMediaChecker(
        ...
        if (dateTaken <= 0) {
            LogUtils.d("创建时间异常 dateTaken: " + dateTaken);
            return false;
        }

        // 创建时间小于开始监听时间
        if (dateTaken < startListenTime) {
            LogUtils.d("开始监听时间校验不通过 dateTaken: " + dateTaken + ", startListenTime: " + startListenTime + ", diff: " + (dateTaken - startListenTime));
            return false;
        }
        ...
        // 文件创建时间超过间隔时间
        if (curTime - dateTaken > intervalTime) {
            LogUtils.d("文件间隔时间校验不通过 dateTaken: " + dateTaken + ", curTime: " + curTime + ", intervalTime: " + intervalTime + ", diff: " + (dateTaken - startListenTime));
            return false;
        }
        ...
        if (checkPrefix) {
            ...
            if (!checkResult) {
                LogUtils.d("文件前缀校验不通过 dataPath: " + dataPath + ", fileName: " + fileName + ", keyWork: " + keyWork);
                return false;
            }
        }
