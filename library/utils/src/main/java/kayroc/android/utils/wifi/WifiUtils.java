        ...
        try {
            ...
            if (isStatic && !TextUtils.isEmpty(ip)) {
                ...
                if (staticWifiConfig == null) {
                    ...
                    // =
                    LogUtils.d("属于正常方式连接 (DHCP)");
                } else {
                    // 设置静态信息
                    connWifiConfig = staticWifiConfig;
                    // =
                    LogUtils.d("属于静态 IP 方式连接");
                }
            } else {
                // 创建正常的配置信息
                connWifiConfig = createWifiConfig(ssid, pwd, type, true);
            ...
            if (preWifiConfig != null) {
                ...
                // 打印结果
                LogUtils.d("删除旧的配置信息 " + preWifiConfig.SSID + ", isRemove: " + isRemove);
                // 保存配置
            ...
            if (nId != -1) {
                try {
                    ...
                    // 打印断开连接结果
                    LogUtils.d("isDisConnect: " + isDisConnect + ", isDisable: " + isDisable);
                } catch (Exception e) {
                    LogUtils.e(e.getMessage());
                }
                // 开始连接
                ...
                // 打印结果
                LogUtils.d("addNetwork(enableNetwork) result: " + result);
            } else {
                // 尝试不带引号 SSID 连接
            ...
            // 连接的 networkId
            LogUtils.d("连接的 SSID networkId: " + nId);
            // 返回连接的信息
