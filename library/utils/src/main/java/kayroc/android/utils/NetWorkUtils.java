 ...
 * </pre>
 */
@SuppressWarnings({"deprecation", "unused", "SpellCheckingInspection", "AlibabaUndefineMagicConstant"})
    ...
    public static boolean setMobileDataEnabled(final boolean isOpen) {
        try {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                ...
                // 需要 Manifest.permission.MODIFY_PHONE_STATE 权限, 普通 APP 无法获取
                TelephonyManager telephonyManager = AppUtils.getTelephonyManager();
                // 通过反射设置移动网络
            ...
            try {
                ...
                if (network != null) {
                    ...
                    if (networkCapabilities.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)) {
                        ...
                        // 获取移动网络类型
                        if (getNetworkClass(networkType) == 1) {
                            // 2G
                            netType = NetworkType.NETWORK_2G;
                        } else if (getNetworkClass(networkType) == 2) {
                            // 3G
                            netType = NetworkType.NETWORK_3G;
                        } else if (getNetworkClass(networkType) == 3) {
                            // 4G
                            netType = NetworkType.NETWORK_4G;
                        } else if (getNetworkClass(networkType) == 4) {
                            // 5G
                            netType = NetworkType.NETWORK_5G;
                        }
                    }
