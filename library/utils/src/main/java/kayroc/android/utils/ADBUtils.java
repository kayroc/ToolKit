...
import kayroc.java.utils.CollectionUtils;
import kayroc.java.utils.FileUtils;
import kayroc.java.utils.StringUtils;
import kayroc.java.utils.RegexUtils;
 ...
 * </pre>
 */
@SuppressWarnings({"unused", "SpellCheckingInspection", "AlibabaUndefineMagicConstant"})
public final class ADBUtils {
    ...
    public static List<String> getAppList(final String type) {
        ...
        if (result.isSuccess3()) {
            try {
                String[] arrays = result.successMsg.split(StringUtils.NEW_LINE_STR);
    ...
    public static int getVersionCode(final String packageName) {
        ...
        try {
            ...
            if (result.isSuccess3()) {
                String[] arrays = result.successMsg.split(RegexUtils.REGEX_SPACE);
        ...
        if (result.isSuccess3()) {
            ...
            if (start != -1) {
                try {
                    ...
                    // 进行拆分
                    String[] arrays = subData.split(StringUtils.NEW_LINE_STR);
                    for (String str : arrays) {
                        if (!TextUtils.isEmpty(str)) {
                            // 存在包名才处理
                            if (str.contains(packageName)) {
                                String[] splitArrays = str.split(RegexUtils.REGEX_SPACE);
    ...
    public static List<String> getActivitysToPackageLists(final String packageName) {
        ...
        if (!TextUtils.isEmpty(result)) {
            try {
                ...
                String[] dataSplit = result.split(StringUtils.NEW_LINE_STR);
                // 拆分后, 数据长度
                ...
                for (String data : activityArrays) {
                    try {
                        String[] splitArrays = data.split(RegexUtils.REGEX_SPACE);
                        if (splitArrays.length != 0) {
                            for (String splitStr : splitArrays) {
    ...
    public static boolean startActivity(
        ...
        if (StringUtils.isSpace(packageAndLauncher)) {
            return false;
        }
        try {
    ...
    public static boolean startService(
        ...
        if (StringUtils.isSpace(packageAndService)) {
            return false;
        }
        try {
    ...
    public static boolean sendBroadcastToAll(final String broadcast) {
        if (StringUtils.isSpace(broadcast)) {
            return false;
        }
        try {
    ...
    public static boolean sendBroadcast(
        ...
        if (StringUtils.isSpace(packageAndBroadcast)) {
            return false;
        }
        if (StringUtils.isSpace(broadcast)) {
            return false;
...
//    }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (result.isSuccess3()) {
                try {
                    ...
                    subStr = subStr.substring(index + 1);
                    // 最后进行添加
                    builder.append(subStr.split(RegexUtils.REGEX_SPACE)[0]);
                    // 返回对应的数据
            ...
            if (result.isSuccess3()) {
                // 返回值中的 Device ID 就是 IMEI
                try {
                    String[] splitArrays = result.successMsg.split(StringUtils.NEW_LINE_STR);
                    for (String str : splitArrays) {
                        if (!TextUtils.isEmpty(str)) {
                            if (str.toLowerCase().contains("device")) {
                                // 进行拆分
                                String[] arrays = str.split(RegexUtils.REGEX_SPACE);
    ...
    public static boolean openAccessibility(
        ...
        if (StringUtils.isSpace(accessibilityServiceName)) {
            return false;
        }
