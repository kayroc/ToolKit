    ...
    public static CommandResult execCmd(
        ...
        try {
            ...
            for (String command : commands) {
                ...
                dos.writeBytes(StringUtils.NEW_LINE_STR);
                dos.flush();
            }
            dos.writeBytes("exit" + StringUtils.NEW_LINE_STR);
            dos.flush();
            // 为了避免 Process.waitFor() 导致主线程堵塞问题, 最好读取信息
            // 如果程序不断在向输出流和错误流写数据, 而 JVM 不读取的话, 当缓冲区满之后将无法继续写入数据, 最终造成阻塞在 waitFor() 这里
            if (isNeedResultMsg) {
                // 读取成功数据
        ...
        try {
            ...
            if ((str = br.readLine()) != null) {
                ...
                while ((str = br.readLine()) != null) {
                    builder.append(StringUtils.NEW_LINE_STR).append(str);
                }
            }
