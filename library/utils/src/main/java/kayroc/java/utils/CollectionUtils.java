...
/**
 * 集合工具类 ( Collection - List、Set、Queue ) 等
 *
 * @author kayroc
 * <pre>
...
public final class CollectionUtils {
    ...
    public static <T> Collection<T> subtract(
        ...
        try {
            ...
            Set<T> sets = new LinkedHashSet<>(collection1);
            // 进行移除
            sets.removeAll(Objects.requireNonNull(intersectionC));
            // 返回集合
            return sets;
        } catch (Exception e) {
            LogUtils.e(e.getMessage());
    ...
    public static class ArrayWithTypeToken<T> {
        ...
        private void newInstance(final Collection<T> collection) {
            ...
            newInstance((Class<T>) Objects.requireNonNull(value).getClass(), objects.length);
            // 保存数据
