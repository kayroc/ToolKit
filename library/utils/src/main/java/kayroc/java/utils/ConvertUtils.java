 ...
 * </pre>
 */
@SuppressWarnings({"unused", "CommentedOutCode", "SpellCheckingInspection"})
public final class ConvertUtils {
    ...
    public static char charAt(
        ...
        try {
            return Objects.requireNonNull(toChars(value))[pos];
        } catch (Exception e) {
            LogUtils.e(e.getMessage());
        }
        return defaultValue;
    ...
    private static String toHexString(
        ...
        try {
            ...
            for (byte datum : data) {
                builder.append(hexDigits[(datum & 0xf0) >>> 4]);
                builder.append(hexDigits[datum & 0x0f]);
            }
            return builder.toString();
        } catch (Exception e) {
            LogUtils.e(e.getMessage());
    ...
    public static void bytesBitwiseAND(final byte[] data) {
        ...
        for (int i = 0, len = data.length; i < len; i++) {
            int d = data[i];
            // 按位补运算符, 翻转操作数的每一位, 即 0 变成 1, 1 变成 0, 再通过反转后的二进制初始化回十六进制
            d = ~d;
            data[i] = (byte) d;
        }
    }
