...
public final class ChineseUtils {
     ...
     * @return 数字中文化字符串
     */
    @SuppressWarnings({"ConstantConditions", "AlibabaAvoidComplexCondition"})
    private static String numberToCHNNumber(
        ...
        for (int i = NUMBER_UNITS.length - 1; i > 0; i--) {
            if (number.doubleValue() >= NUMBER_UNITS[i]) {
                // 倍数
                final int multiple = (int) (number.doubleValue() / NUMBER_UNITS[i]);
                // 递减
                number = number.subtract(BigDecimal.valueOf(multiple * NUMBER_UNITS[i]));
                // 如果倍数大于万倍, 则直接递归
                if (multiple >= 10000) {
                    // 拼接数值
    ...
    private static String thousandConvertCHN(
        ...
        for (int i = 2; i >= 0; i--) {
            if (number >= units[i]) {
                ...
                // 数字单位
                builder.append(chnUnits[TEN_POS + i]);
                // 进行改变处理
                zeros[i] = false;
            }
            // 补零判断处理
