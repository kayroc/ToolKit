...
public final class CalendarUtils {
    ...
    public static boolean isSupportLunar(final int year) {
        return year >= MIN_LUNAR_YEAR && year <= MAX_YEAR;
    }
    ...
    public static boolean isSupportSolar(final int year) {
        return year >= MIN_SOLAR_YEAR && year <= MAX_YEAR;
    }
    ...
    public static Festival getFestival(
        ...
        for (Festival festival : list) {
            if (festival != null) {
                ...
                if (festival.isFestival(month, day)) {
                    return festival;
                }
            }
        }
    ...
    public static class SolarLunar {
        ...
        public SolarLunar(
            ...
            this.day = day;
            // 公历转农历
            int[] lunarArray = CalendarUtils.solarToLunar(year, month, day);
            result = lunarArray != null;
            if (result) {
                lunarYear = lunarArray[0];
                lunarMonth = lunarArray[1];
                lunarDay = lunarArray[2];
                isLunarLeap = lunarArray[3] == 1;
            } else {
                lunarYear = 0;
                lunarMonth = 0;
                lunarDay = 0;
        ...
        public SolarLunar(
            ...
            // 农历转公历
            int[] solarArray = CalendarUtils.lunarToSolar(lunarYear, lunarMonth, lunarDay, isLunarLeap);
            result = solarArray != null;
            if (result) {
                year = solarArray[0];
                month = solarArray[1];
                day = solarArray[2];
    ...
    // 节日 Hook 接口
    private static FestivalHook sFestivalHook = (festival, year, month, day) -> {
        if (festival != null) {
